%% reduced 1 
classes = [0 1 2 4 5];
keep_cols_1 = [5:7 38:46 64:72 77:85  103:134]; 
keep_cols_2 = [5:7  64:72 103:118];
labels_col = 244;
mkdir('conf1')
mkdir('conf2')
...
% 113 are feature columns - last one labels columns


labelled_data1 = zeros(size(data1,1), size(keep_cols_1,2)+1);
data=data1;
for i=1:size(data,1)
        labelled_data1(i,1:end-1) = data(i,keep_cols_1);
        labelled_data1(i,end) = find(classes == data(i,labels_col));
end

labelled_data2 = zeros(size(data2,1), size(keep_cols_1,2)+1);
data=data2;
for i=1:size(data,1)
        labelled_data2(i,1:end-1) = data(i,keep_cols_1);
        labelled_data2(i,end) = find(classes == data(i,labels_col));
end

labelled_data3 = zeros(size(data3,1), size(keep_cols_1,2)+1);
data=data3;
for i=1:size(data,1)
        labelled_data3(i,1:end-1) = data(i,keep_cols_1);
        labelled_data3(i,end) = find(classes == data(i,labels_col));
end

labelled_data4 = zeros(size(data4,1), size(keep_cols_1,2)+1);
data=data4;
for i=1:size(data,1)
        labelled_data4(i,1:end-1) = data(i,keep_cols_1);
        labelled_data4(i,end) = find(classes == data(i,labels_col));
end

labelled_data5 = zeros(size(data5,1), size(keep_cols_1,2)+1);
data=data5;
for i=1:size(data,1)
        labelled_data5(i,1:end-1) = data(i,keep_cols_1);
...
labelled_data1 = zeros(size(data1,1), size(keep_cols_2,2)+1);
data=data1;
for i=1:size(data,1)
        labelled_data1(i,1:end-1) = data(i,keep_cols_2);
        labelled_data1(i,end) = find(classes == data(i,labels_col));
end

labelled_data2 = zeros(size(data2,1), size(keep_cols_2,2)+1);
data=data2;
for i=1:size(data,1)
        labelled_data2(i,1:end-1) = data(i,keep_cols_2);
        labelled_data2(i,end) = find(classes == data(i,labels_col));
end

labelled_data3 = zeros(size(data3,1), size(keep_cols_2,2)+1);
data=data3;
for i=1:size(data,1)
        labelled_data3(i,1:end-1) = data(i,keep_cols_2);
        labelled_data3(i,end) = find(classes == data(i,labels_col));
end

labelled_data4 = zeros(size(data4,1), size(keep_cols_2,2)+1);
data=data4;
for i=1:size(data,1)
        labelled_data4(i,1:end-1) = data(i,keep_cols_2);
        labelled_data4(i,end) = find(classes == data(i,labels_col));
end

labelled_data5 = zeros(size(data5,1), size(keep_cols_2,2)+1);
data=data5;
for i=1:size(data,1)
        labelled_data5(i,1:end-1) = data(i,keep_cols_2);
        labelled_data5(i,end) = find(classes == data(i,labels_col));
end
